...
EXEC sp_rename 'orders.pizza_id', 'row_id' , 'COLUMN';
...
SELECT DISTINCT order_time,
CONVERT(TIME, order_time, 108)
FROM orders
ORDER BY order_time;

UPDATE orders
SET order_time = CONVERT(TIME, order_time, 108)
ALTER TABLE orders
...
UPDATE orders
SET order_datetime = CONCAT(CONVERT(NVARCHAR(255), order_date, 111),
' ',
CONVERT(NVARCHAR(255), order_time, 108));
